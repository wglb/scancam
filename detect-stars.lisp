...
(defun brightness-f (img x y)
  (let ((pix (get-pixel x y :image img)))
	(/ (+ (ldb (byte 8 16) pix) (ldb (byte 8 8) pix) (ldb (byte 8 0) pix)) 3.0)))
			...
			(let ((y-up (- y delt))
			  ...
			  (when (and (plusp y-up2) (< y-down2 height) t)
				(let* ((bright-left (brightness img x-left y))
					   ...
					   (bright-center (brightness-f img x y))
					   (brightf-center (brightness-f img x y))
				  ...
				  (if (> brightf-center (first max-brightf))
					  (setf max-brightf (list bright-center x y)))
				  ...
				  bright-center)))))))
	(xlogntf "fbsi: brightest point ~s at x ~s y ~s" (first max-brightf) (second max-brightf) (third max-brightf))
	(let ((brights nil))
	  (dotimes (x width)
		(dotimes (y height)
		  (let* ((brightf-center (brightness-f img x y)))
			(when (< (- (first max-brightf) brightf-center) 20)
			  #+nil (break "bright center check, diff ~s " (- (first max-brightf) brightf-center))
			  (push (list brightf-center x y) brights)))))
	  (values
...
(defun find-bright-spots (fn delt brightness-diff-thresh cluster-limit overlap-thresh) 
  (with-image-from-file (img fn :jpg)
	(multiple-value-bind (width height)
		(image-size img)
	  (multiple-value-bind (poss max-calc brights)
		  (find-bright-spots-int fn img width height delt brightness-diff-thresh overlap-thresh)
		(xlogntf "Check poss, max-calc   ~s brights ~s" max-calc (length brights))
		(let ((new-directory (append (pathname-directory fn) (list "marked-images"))))
